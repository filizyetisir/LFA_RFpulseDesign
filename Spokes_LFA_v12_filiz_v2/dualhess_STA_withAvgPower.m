...
function h=dualhess_STA_withAvgPower(xrf,lambda,A,adj,spokes,system,sar,solver,do_mls,opt)
...
if(opt.sarcons)
    ...
    nconst=sar.nvop+1;
    subh=2*squeeze( sum(sar.sarmats .* repmat(LagMult(1:nconst),[1 nchannels nchannels]),1 ) );
...
tmp=repmat(tmp,[2*spokes.nspokes 1]); % real and imaginary and spokes
h_AVG_POW=2.0*diag(tmp);


if(opt.sarcons)
    h=h_OBJ + h_SAR + h_MAX_POW;
else
    h=h_OBJ + h_MAX_POW + h_AVG_POW;
end
