...
function [mxf myf mzf]=bloch_simulation_z_correct(spokes,system,zs,b0_off)
...
for time=1:spokes.ntimes
    ...
    if spokes.time_to_spoke(time)<=0 
        b1s=zeros(size(zs,1),1);
    else
        tmp = spokes.time_to_sinc_time(time);
        b1s=repmat( 6.4e-8*spokes.sinc(tmp),size(zs,1),1 ); 
    end
