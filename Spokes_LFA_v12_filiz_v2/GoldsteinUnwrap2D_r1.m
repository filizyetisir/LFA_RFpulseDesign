...
function im_unwrapped=GoldsteinUnwrap2D_r1(IM,varargin)
% function im_unwrapped=GoldsteinUnwrap2D_r1(IM,varargin)
% optional argument: im_mask
...
mag_max = max(im_mag(:));
% figure; imagesc(im_mag.*im_mask),   colormap(hot), axis square, axis off, title('GS Initial masked magnitude'); colorbar;
% figure; imagesc(im_phase.*im_mask), colormap(hot), axis square, axis off, title('GS Initial masked phase'); colorbar;

%% Compute the residues
residue_charge = PhaseResidues_r1(im_phase, im_mask); % Calculate phase residues (Does not use mask)
% figure; imagesc(residue_charge), colormap(hot), axis square, axis off, title('GS Phase residues (charged)'); colorbar;
...
% figure; imagesc(branch_cuts),    colormap(hot), axis square, axis off, title('GS Branch cuts'); colorbar;
...
if(0)  % Chose starting point interactively
  ...
  figure; imagesc(im_phase_quality,[minp maxp]), colormap(hot), colorbar, axis square, axis off; title('Phase quality map');
  ...
  im_mag1(:,1) = 0;
  im_mag1(:,c_dim) = 0;
  [rowrefn,colrefn] = find(im_mag1 >= 0.99*max(im_mag1(:)));
...
% Display results
% figure; imagesc(im_unwrapped), colormap(hot), colorbar, axis square, axis off, title('GS Unwrapped phase');
